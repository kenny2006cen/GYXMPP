...
- (void)setup {
  ...
  UILongPressGestureRecognizer *longTap = [[UILongPressGestureRecognizer alloc]
      initWithTarget:self
              action:@selector(chatbackgroundViewLongtap:)];
  [chatbackgroundView addGestureRecognizer:longTap];
  UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]
      initWithTarget:self
              action:@selector(chatbackgroundViewtap:)];
  ...
  for (int i = 1; i <= 3; i++) {
    [audioImageArray
        addObject:[UIImage imageNamed:[NSString
                                          stringWithFormat:@"hd_audio_right_%d",
                                                           i]]];
  }
  UIImageView *chatAudioImageView = [[UIImageView alloc] init];
  chatAudioImageView.image = [UIImage imageNamed:@"hd_audio_right_1"];
  ...
  chatAudioTimeLabel.font = [UIFont systemFontOfSize:12.0];
...
- (void)setChatModel:(GYHDNewChatModel *)chatModel {
  ...
  if (dataDict[@"mp3Len"] && dataDict[@"mp3"]) {
    self.chatAudioTimeLabel.text =
        [NSString stringWithFormat:@"%d\"", [dataDict[@"mp3Len"] integerValue]];
    audioW = 40 + [dataDict[@"mp3Len"] integerValue] * 5;
  } else {
    self.chatAudioTimeLabel.text = [NSString
        stringWithFormat:@"%d\"", [dict[@"msg_fileSize"] integerValue]];
  ...
  //    [self.chatbackgroundView mas_remakeConstraints:^(MASConstraintMaker
  //    *make) {
  ...
  //    }];
  //
  //    [self.chatAudioTimeLabel mas_remakeConstraints:^(MASConstraintMaker
  //    *make) {
  //        make.centerY.equalTo(self.chatbackgroundView);
  //        make.right.equalTo(self.chatbackgroundView.mas_left).offset(-5);
  //    }];
  //
  //    [self.chatAudioImageView mas_remakeConstraints:^(MASConstraintMaker
  //    *make) {
  //        make.centerY.equalTo(self.chatbackgroundView);
...
- (void)chatbackgroundViewLongtap:(UILongPressGestureRecognizer *)longTap {
  if (longTap.state == UIGestureRecognizerStateBegan) {
    ...
    //        [menu setTargetRect:self.chatbackgroundView.bounds
    //        inView:self.chatbackgroundView];
...
- (void)deleteItemClicked:(id)sender {
  if ([self.delegate respondsToSelector:@selector(GYHDChatView:
                                                 ...
                                                 chatMessageID:)]) {
    [self.delegate GYHDChatView:self
                    longTapType:GYHDChatTapChatAudio
