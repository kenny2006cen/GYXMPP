...
//  HMBaseKit
//
//  Created by Ko Lee on 2020/5/17.
...
public class HMBaseDevice {
    
    
    public static var fuckValue: String {
     ...
     public static var systemVersion: String {
         return HMBaseDeviceSystem.systemVersion
     }
     
     ///获取系统名称
     public static var systemName: String {
         return HMBaseDeviceSystem.systemName
     }
     
     ///获取系统名称 iPhone", "iPod touch"
     public static var deviceModel: String {
         return HMBaseDeviceSystem.deviceModel
     }
     
     ///获取系统名称 localized version of model
     public static var deviceLocalizedModel: String {
         return HMBaseDeviceSystem.deviceLocalizedModel
     }
     
     ///获取设备名称 如 XXX的iphone
     public static var deviceUserName: String {
         return HMBaseDeviceSystem.deviceUserName
     }
     ///获取总的内存
     public static var deviceDiskTotalSize: String {
         return HMBaseDeviceSystem.deviceDiskTotalSize
     }
     
     ///获取可用的内存
     public static var deviceAvalibleDiskSize: String {
         return HMBaseDeviceSystem.deviceAvalibleDiskSize
     }
     
     ///获取运营商
     public static var supplier: String {
         return HMBaseDeviceSystem.supplier
     }
     
     /// 获取当前设备IP
     public static var deviceIP: String {
         return HMBaseDeviceSystem.deviceIP
     }
    
     
     ///获取cpu核数
     public static var deviceCpuCount: Int {
         return HMBaseDeviceSystem.deviceCpuCount
     }
     
     ///获取cpu类型
     public static var deviceCpuType: String {
         return HMBaseDeviceSystem.deviceCpuType
     }
     
     ///获取设备名称
     public static var deviceName: String {
         return HMBaseDeviceSystem.deviceName
     }
     ...
     public static var appBundleName:  String {
         return hm_basekit_appBundleName()
     }
...
private extension HMBaseDevice {
    
    // MARK: --------------------------- 设备系统信息 ---------------------
    
    ///App名称 获取失败则返回空字符串
     private class func hm_basekit_appName() -> String  {
    ...
    /// App包名 获取失败则返回空字符串
     private class func hm_basekit_appBundleId() -> String  {
        if let identifier = Bundle.main.bundleIdentifier {
            return identifier
        }
        return ""
    ...
    /// App版本号 获取失败则返回空字符串
     private class func hm_basekit_appVersion() -> String  {
        if let bundleDic = Bundle.main.infoDictionary, let varsion = bundleDic["CFBundleShortVersionString"] {
            return (varsion as! String)
        }
        return ""
    ...
    /// AppIdfa 用户关闭，则返回空字符串
     private class func hm_basekit_appIdfa() -> String {
        if ASIdentifierManager.shared().isAdvertisingTrackingEnabled {
            return ASIdentifierManager.shared().advertisingIdentifier.uuidString
        }
        return ""
    ...
    /// AppIdfv 获取失败则返回空字符串
     private class func hm_basekit_appIdfv() -> String {
        if let idForVendor = UIDevice.current.identifierForVendor {
            return idForVendor.uuidString
        }
        return ""
    ...
    ///app工程名称 获取失败则返回空字符串
     private class func hm_basekit_appBundleName() -> String {
    ...
    private class func hm_basekit_currentPhoneModelSize() -> CGSize {
    ...
    private class func hm_basekit_phoneEqualTo(_ size: CGSize) -> Bool {
        return hm_basekit_currentPhoneModelSize().equalTo(size)
    }

    //MARK: ----  判断IPad ----
    
    private class func hm_basekit_isIPadType() -> Bool {
    ...
    private class func hm_basekit_isPhoneXType() -> Bool {
        if (hm_basekit_iPhoneInch65() || hm_basekit_iPhoneInch58() || hm_basekit_iPhoneInch61()) {
            return true
        }
        return false
    }
    ...
    private class func hm_basekit_iPhoneInch65() -> Bool {
        guard (hm_basekit_isIPadType() == false) else { return false }
        return hm_basekit_phoneEqualTo(CGSize(width: 1242.0, height: 2688.0))
    }

    //MARK: ----  判断iPhone6.1 iPhone XR ----
    private class func hm_basekit_iPhoneInch61() -> Bool {
        guard (hm_basekit_isIPadType() == false) else { return false }
        return hm_basekit_phoneEqualTo(CGSize(width: 828.0, height: 1792.0))
    }

    //MARK: ----  判断iPhone5.8 iPhone XS/ iPhone X ----
    
    private class func hm_basekit_iPhoneInch58() -> Bool {
        guard (hm_basekit_isIPadType() == false) else { return false }
        return hm_basekit_phoneEqualTo(CGSize(width: 1125.0, height: 2436.0))
    }

    //MARK: ----  判断iPhone5.5 iPhone 8 Plus/ iPhone X ----
    
    private class func hm_basekit_iPhoneInch55() -> Bool {
        guard (hm_basekit_isIPadType() == false) else { return false }
        return hm_basekit_phoneEqualTo(CGSize(width: 1242.0, height: 2208.0))
    }

    //MARK: ----  判断iPhone4.7 iPhone 6/6s/7/8 ----
    
   private class func hm_basekit_iPhoneInch47() -> Bool {
        guard (hm_basekit_isIPadType() == false) else { return false }
        return hm_basekit_phoneEqualTo(CGSize(width: 750.0, height: 1334.0))
    }

    //MARK: ----  判断iPhone4 iPhone SE ----
    
    private class func hm_basekit_iPhoneInch4() -> Bool {
        guard (hm_basekit_isIPadType() == false) else { return false }
        return hm_basekit_phoneEqualTo(CGSize(width: 640.0, height: 1136.0))
    }
}
