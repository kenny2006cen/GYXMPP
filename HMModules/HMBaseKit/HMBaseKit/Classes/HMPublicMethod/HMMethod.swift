...
public class HMMethod {
    ...
    public class func originalImage(_ imageName: String) -> UIImage? {
        return hm_basekit_originalImage(imageName)
    }
    
    ///等比缩放图片获取高度
    public class func imageHeight(_ imageW: CGFloat, _ imageH: CGFloat, _ realW: CGFloat) -> CGFloat {
        return hm_basekit_imageHeight(imageW, imageH, realW)
    }
    
    ///根据root控制器，返回当前控制器
    public class func currentVC() -> UIViewController? {
        return hm_basekit_currentVC()
    }
    
    ///根据颜色生成图片（UIImage）
    public class func imageWithColor(_ color: UIColor, _ size: CGSize) -> UIImage {
        return hm_basekit_createImageFrom(color, size)
    }
    
    ///打开链接  OpenUrl
    @discardableResult
    public class func openUrl(_ url: URL) -> Bool {
        return hm_basekit_openUrl(url)
    }
    
    ///拨打电话
    public class func hm_callPhone(_ number: String) {
        hm_basekit_callPhone(number)
    }
    
    /// 跳转appStore 评论
    public class func hm_appStoreComment(_ appId: String) {
        hm_basekit_appStoreComment(appId)
    }
    ...
    {
       return hm_basekit_moneyFormatter(money: money, maxFractionDigits: maxFractionDigits, prefix: prefix, separator: separator, groupingSize: groupingSize)
    }
    ...
    public class func amountFormatter(_ money: Double, _ maxFractionDigits: Int, _ prefix:String) -> String {
        return hm_basekit_amountFormatter(money: money, maxFractionDigits: maxFractionDigits, prefix: prefix)
    }
    
    ///跳转Dic -> JsonStr
    public class func dicToJsonStr(_ dic: [String: Any]) -> String? {
        return hm_basekit_dicToJsonStr(dic)
    }
    
    ///图片旋转90度
    public class func rotationImage90(_ imageName: String) -> UIImage? {
        return hm_basekit_rotationImage90(imageName)
    }
    ...
    public class func characterSetWithStr(_ characterStr: String, _ judgeText: String) -> Bool {
        return hm_basekit_characterSetWithStr(characterStr, judgeText)
    }
    ...
    */
    public class func compressImage(_ image: UIImage, _ maxLength: Int, _ size:CGSize) -> Data? {
        return hm_basekit_compressImage(image: image, maxLength: maxLength, size: size)
    }
    ...
    public class func reSetSizeImage(_ size:CGSize, _ image: UIImage) -> UIImage {
        return hm_basekit_reSizeImage(reSize: size, image: image)
    }
    
    ///json字符串去除空格
    public class func jsonStrRemoveSpace(_ jsonStr: String) -> String {
        return hm_basekit_jsonStrRemoveSpace(jsonStr)
    }
    
    ///url字符串utf-8编码
    public class func urlEncodingToUtf8(_ str: String) ->  String {
        return hm_basekit_jstrEncodingToUtf8(str)
    }
}


private extension HMMethod {
    
    class func hm_basekit_jstrEncodingToUtf8(_ str: String) ->  String {
        if str.count > 0 {
            let userved = "!NULL,'()*+,-./:;=?@_~%#[]"
    ...
    class func hm_basekit_jsonStrRemoveSpace(_ jsonStr: String) -> String {
          var jsonStr = jsonStr
     ...
     *  return 压缩后图片的二进制
     */
    class func hm_basekit_compressImage(image: UIImage, maxLength: Int, size:CGSize) -> Data? {
        let newImage =  hm_basekit_reSizeImage(reSize: CGSize(width: size.width, height: size.height), image: image)
    ...
    /// 尺寸的重置
    class func hm_basekit_reSizeImage(reSize:CGSize, image: UIImage) -> UIImage {
    ...
    ///判断文字的格式是否满足
    class func hm_basekit_characterSetWithStr(_ characterStr: String, _ judgeText: String) -> Bool {
    ...
    ///图片旋转90度
    class func hm_basekit_rotationImage90(_ imageName: String) -> UIImage? {
    ...
    ///跳转Dic -> JsonStr
    class func hm_basekit_dicToJsonStr(_ dic: [String: Any]) -> String? {
        do {
    ...
    ///跳转appStore评分
    class func hm_basekit_appStoreComment(_ appId: String) {
        let urlStr = "itms-apps://itunes.apple.com/app/id\(appId)?action=write-review"
        if let openUrl = URL(string: urlStr) {
            hm_basekit_openUrl(openUrl)
        }
    }
    
    ///拨打电话
    class func hm_basekit_callPhone(_ number: String) {
        let urlString = "tel://\(number)"
        if let url = URL(string: urlString) {
            hm_basekit_openUrl(url)
        }
    }
    
    ///返回原始图片
    class func hm_basekit_originalImage(_ imageName: String) -> UIImage? {
    ...
    ///等比缩放图片获取高度
    class func hm_basekit_imageHeight(_ imageW: CGFloat, _ imageH: CGFloat, _ realW: CGFloat) -> CGFloat {
        return (realW * imageH) / imageW
    }
    
    /// 根据控制器，返回当前控制器
    class func hm_basekit_currentVC() -> UIViewController? {
        guard let rootVC = UIApplication.shared.keyWindow?.rootViewController else {
            return nil
        }
        return hm_basekit_mapCurrentVC(rootVC: rootVC)
    }
    
    /// 递归找最上面的控制器
    class func hm_basekit_mapCurrentVC(rootVC :UIViewController) -> UIViewController? {
        var currentVC: UIViewController?
        if rootVC.presentedViewController != nil {
            currentVC = rootVC.presentedViewController
        } else if rootVC.isKind(of: UITabBarController.self) == true {
            currentVC =  hm_basekit_mapCurrentVC(rootVC: (rootVC as! UITabBarController).selectedViewController!)
        } else if rootVC.isKind(of: UINavigationController.self) == true {
            currentVC = hm_basekit_mapCurrentVC(rootVC: (rootVC as! UINavigationController).visibleViewController!)
        } else {
            currentVC = rootVC
        }
        return currentVC
   ...
   class func hm_basekit_createImageFrom(_ color: UIColor, _ size: CGSize) -> UIImage {
    ...
    ///打开链接  OpenUrl
    @discardableResult
    class func hm_basekit_openUrl(_ url: URL) -> Bool {
    ...
    /// 金额格式化用组分隔 prefix 自定义前缀(如 $/￥)  separator 分隔符号（如,） groupingSize 分隔位数 maxFractionDigits 小数点后最多位数
   class func hm_basekit_moneyFormatter(money: Double, maxFractionDigits: Int, prefix:String,  separator: String, groupingSize: Int) -> String
    {
    ...
    /// 金额格式化不用组分隔 prefix 自定义前缀(如 $/￥)  maxFractionDigits 小数点后最多位数
    class func hm_basekit_amountFormatter(money: Double, maxFractionDigits: Int, prefix:String) -> String
     {
         let number = NSNumber(value: money)
         let formatter = NumberFormatter()
         // 设置显示样式
