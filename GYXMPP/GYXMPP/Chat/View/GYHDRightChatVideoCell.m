...
- (void)setChatModel:(GYHDNewChatModel *)chatModel {
  ...
  [self.iconImageView setImageWithURL:[NSURL URLWithString:globalData.loginModel.headPic] placeholder:kLoadPng(@"defaultheadimg") options:kNilOptions completion:nil];
  ...
  // iphone4 录制视频崩溃信息如下modify by jianglincen
  ...
  NSString *thumbnailsImageNamePath = [NSString pathWithComponents:@[
    ...
    videoDict[@"thumbnailsName"] ? videoDict[@"thumbnailsName"] : @""
  ]];
  ...
  if (image) {
    ...
    [self.chatPictureImageView
        setImageWithURL:[NSURL URLWithString:bodyDict[@"msg_imageNail"]] placeholder:[UIImage imageNamed:@"placeholder_image"] options:kNilOptions completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
            CALayer *layer = ImageView.layer;
            layer.frame = (CGRect){{0, 0}, ImageView.layer.frame.size};
            self.chatPictureImageView.layer.mask = layer;
        } ];
