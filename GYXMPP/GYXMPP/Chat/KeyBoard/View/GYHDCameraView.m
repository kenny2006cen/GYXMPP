...
- (void)layoutSubviews {
  ...
  self.recordCamera =
      [[GYHDRecordVideoTool alloc] initWithCameraView:self.showCameraView];
}
- (void)setup {
  // 3. 显示界面
  ...
  recordPhotoView.backgroundColor =
  ...
  [recordPhotoButton setBackgroundImage:[UIImage imageNamed:@"btn-ps_n"]
                               forState:UIControlStateNormal];
  [recordPhotoButton setBackgroundImage:[UIImage imageNamed:@"btn-ps_h"]
                               forState:UIControlStateHighlighted];
  [recordPhotoButton addTarget:self
                        action:@selector(recordPhotoButtonClick)
              forControlEvents:UIControlEventTouchUpInside];
...
- (void)recordPhotoButtonClick {
 // DDLogCError(@"拍照");
  ...
  //此处有问题，startRecordCamera 没有回调，只是延迟获取拍照后的imageData数据
  dispatch_after(
...
- (void)GYHDCameraShowView:(UIView *)cameraShowView
  ...
  if ([self.delegate
    ...
    [self.delegate GYHDKeyboardSelectBaseView:self
                                     sendDict:[self.recordCamera imagePathDict]
