 ...
 * Description    : 计算一块缓冲区的crc32校验值
 * Input          : None
 * Output         : None
 * Return         : None
 * Attention  : None
 *******************************************************************************/
unsigned int crc_check( uint8_t *buf , int size )
{
    unsigned int crc = 0xffffffff;
    crc = crc32( crc , (uint8_t *)buf , size );
    return crc;
}


uint32_t make_crc(uint32_t crc, uint8_t *buf, uint32_t size)
{
...
- (unsigned int)hm_crc32
{
    NSData *data = self;
    
    uint8_t *bytes = (uint8_t *)[data bytes];
    
    return crc_check(bytes, (int)data.length);
}
@end
