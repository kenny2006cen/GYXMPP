...
- (void)setup {
  ...
  [navbutton setImage:[UIImage imageNamed:@"hd_nav_leftView_back"]
...
- (void)sendButtonClick {
  if ([self.delegate
          respondsToSelector:@selector(GYHDPhotoShowView:SendImageData:)]) {
    UIImage *image = nil;

    if (self.photoImageView.image.size.width > 1080 ||
        self.photoImageView.image.size.height > 1080) {
     // image = [Utils imageCompressForWidth:self.photoImageView.image
      //                         targetWidth:1080];
