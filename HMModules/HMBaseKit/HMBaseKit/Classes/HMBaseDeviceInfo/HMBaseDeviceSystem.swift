...
class HMBaseDeviceSystem: NSObject {
    ...
    ///获取系统版本
     ...
     static var deviceModel: String {
      ...
      static var deviceDiskTotalSize: String {
          return hm_basekit_fileSizeToString(fileSize: hm_basekit_getTotalDiskSize())
      }
      
      ///获取可用的内存
      static var deviceAvalibleDiskSize: String {
          return hm_basekit_fileSizeToString(fileSize: hm_basekit_getAvailableDiskSize())
      }
      
      ///获取运营商
      static var supplier: String {
          return hm_basekit_deviceSupplier()
      }
      
      /// 获取当前设备IP
      static var deviceIP: String {
          return hm_basekit_deviceIP()
      }
      
      ///获取cpu核数
      static var deviceCpuCount: Int {
          return hm_basekit_deviceCpuCount()
      }
      
      ///获取cpu类型
      static var deviceCpuType: String {
          return hm_basekit_deviceCpuType()
      }
      
      ///获取设备名称
      static var deviceName: String {
          return hm_basekit_deviceName()
      }
}
...
private extension HMBaseDeviceSystem {
    
    //MARK:  ---  获取运营商
    
    private class func hm_basekit_deviceSupplier() -> String {
    ...
    private class func hm_basekit_deviceIP() -> String {
    ...
    private class func hm_basekit_deviceCpuCount() -> Int {
    ...
    private class func hm_basekit_deviceCpuType() -> String {
    ...
    private class func hm_basekit_deviceName() -> String {
    ...
    private class func hm_basekit_blankof<T>(type:T.Type) -> T {
    ...
    private class func hm_basekit_getTotalDiskSize() -> Int64 {
        var fs = hm_basekit_blankof(type: statfs.self)
        if statfs("/var",&fs) >= 0{
    ...
    private class func hm_basekit_getAvailableDiskSize() -> Int64 {
        var fs = hm_basekit_blankof(type: statfs.self)
        if statfs("/var",&fs) >= 0{
    ...
    private class func hm_basekit_fileSizeToString(fileSize:Int64) -> String {
